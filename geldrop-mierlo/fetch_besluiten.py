#!/usr/bin/env python3
"""
Haal alle 2025 Gemeenteblad-publicaties op voor Geldrop-Mierlo
via de SRU API van repository.overheid.nl en genereer data.js
"""
...
SUBJECT_TO_DOMEIN = {
    'Ruimte en infrastructuur': 'Ruimte, Wonen & Mobiliteit',
    'Verkeer': 'Ruimte, Wonen & Mobiliteit',
    'Natuur en milieu': 'Duurzaamheid, Natuur & Leefbaarheid',
    'Landbouw': 'Duurzaamheid, Natuur & Leefbaarheid',
    'Bestuur': 'Bestuur & Veiligheid',
    'Openbare orde en veiligheid': 'Bestuur & Veiligheid',
    'Recht': 'Bestuur & Veiligheid',
    'Financiën': 'Financiën & Economie',
    'Economie': 'Financiën & Economie',
    'Cultuur en recreatie': 'Onderwijs, Sport & Cultuur',
    'Onderwijs en wetenschap': 'Onderwijs, Sport & Cultuur',
    'Zorg en gezondheid': 'Sociaal Domein & Zorg',
    'Sociale zekerheid': 'Sociaal Domein & Zorg',
    'Migratie en integratie': 'Bestuur & Veiligheid',
    'Werk': 'Sociaal Domein & Zorg',
...
def fetch_year(year):
    query = f"(c.product-area==officielepublicaties AND dt.creator==Geldrop-Mierlo AND w.publicatienaam==Gemeenteblad AND w.jaargang=={year})"
    records = []
    start = 1
    total = None
...
def main():
    print("Ophalen besluiten Geldrop-Mierlo 2025...", file=sys.stderr)
    records = fetch_all()
    print(f"\n  Opgehaald: {len(records)} besluiten (na filtering)", file=sys.stderr)

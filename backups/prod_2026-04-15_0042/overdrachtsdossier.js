...
(function () {
    ...
    /* ── TOC-links: open parent <details> en scroll naar doel ── */
    document.querySelectorAll('.overdrachtsdossier-toc-link').forEach(function (link) {
        link.addEventListener('click', function (e) {
            ...
            // Open alle <details>-voorouders van het doel
            var node = target;
            while (node) {
                if (node.tagName === 'DETAILS') node.open = true;
                node = node.parentElement;
            }
            // Open het doel zelf als het een <details> is
            if (target.tagName === 'DETAILS') target.open = true;

            // Sluit het mobiele menu als dat open is
